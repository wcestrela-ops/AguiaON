  ...
  try {
    if (type === 'smtp') {
      ...
      await transporter.sendMail({
        from, to,
        subject: 'Teste de Configuração AG-ON',
        text: 'Se você recebeu este e-mail, seu SMTP está configurado corretamente no AG-ON!'
      });
    ...
    if (type === 'firebase') {
      ...
      const reqFetch = await fetch('https://fcm.googleapis.com/fcm/send', {
        ...
        body: JSON.stringify({
          ...
          notification: { title: 'AG-ON Test', body: 'Teste de conectividade FCM' },
        }),
      });
    ...
    if (type === 'wa_otp') {
      ...
      const reqFetch = await fetch(`${cleanUrl}/message/sendText/${instance}`, {
        ...
        body: JSON.stringify({ number, text: '*AG-ON* — Teste de OTP via WhatsApp funcionando corretamente!' }),
      });
      if (!reqFetch.ok) {
...
router.post('/seed-demos', requireAdmin, async (_req, res) => {
  ...
  const DEMOS = [
    ...
    },
    {
      ...
      active_features: ['frota', 'cobranca', 'crm', 'vendas'],
