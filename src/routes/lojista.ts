...
(async () => {
  try {
    ...
    // whatsapp_configs: mesma classe de bug já encontrada antes (user_addresses,
    // business_hours) — 12 colunas usadas em GET/PUT /lojista/whatsapp/config e
    // em GET /lojista/onboarding-status (instance_name), mas nenhuma tinha
    // CREATE/ALTER em lugar nenhum do projeto — só existiam no
    // migration_v6_whatsapp_automation.sql original (sem essas colunas) e
    // presumivelmente foram adicionadas manualmente no banco antigo de
    // produção, fora de controle de versão. Achado investigando o 500 real
    // em /lojista/onboarding-status reportado pelo Carlos (relation/column
    // "instance_name" — na real "whatsapp_configs.instance_name" — does not exist).
    // Garante a tabela em si também (defensivo — mesmo padrão do
    // migration_v6_whatsapp_automation.sql original, caso essa migration
    // nunca tenha rodado manualmente neste banco).
    await pool.query(`
    ...
    `);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS api_type TEXT DEFAULT 'evolution'`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS custom_api_url TEXT`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS custom_api_token TEXT`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS instance_name TEXT`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS msg_new_order TEXT`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS msg_confirmed TEXT`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS msg_preparing TEXT`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS msg_delivering TEXT`);
    await pool.query(`ALTER TABLE whatsapp_configs ADD COLUMN IF NOT EXISTS msg_ready_delivery TEXT`);
    ...
    // usadas em payments.ts (callback OAuth, failover de Pix), pixService.ts
    // e public.ts, mas só `mp_public_key` tinha um ALTER defensivo isolado
    // (em payments.ts, na hora do callback) — as outras quatro nunca tinham
    // CREATE/ALTER em lugar nenhum. Achado no log real de produção:
    // "[lojista/me] colunas financeiras indisponíveis: column mp_status does
    // not exist" — a SELECT de colunas financeiras de /lojista/me falhava
    // inteira por causa dessa única coluna faltando (isolado desde o Fix de
    // produção 4, então não derruba mais vertical_slug/active_features, mas
    // ainda impedia asaas_api_key/pix_key_type/etc. de carregar no painel).
    await pool.query(`ALTER TABLE establishments ADD COLUMN IF NOT EXISTS mp_access_token TEXT`);
    await pool.query(`ALTER TABLE establishments ADD COLUMN IF NOT EXISTS mp_refresh_token TEXT`);
    await pool.query(`ALTER TABLE establishments ADD COLUMN IF NOT EXISTS mp_user_id TEXT`);
    await pool.query(`ALTER TABLE establishments ADD COLUMN IF NOT EXISTS mp_public_key TEXT`);
    await pool.query(`ALTER TABLE establishments ADD COLUMN IF NOT EXISTS mp_status BOOLEAN NOT NULL DEFAULT false`);
  } catch (e: any) {
...
router.get('/me', async (req, res) => {
  ...
  try {
    // Busca só as colunas críticas primeiro (as que a navegação do painel
    // depende: vertical_slug/active_features decidem quais abas aparecem).
    // Antes isso vinha numa SELECT só com ~25 colunas — se QUALQUER uma
    // faltasse (ex: coluna de uma migration que só roda manualmente, tipo
    // migration_v5_triple_play.sql), a query inteira falhava e o catch-all
    // devolvia um "sucesso" falso com vertical_slug='generico' e
    // active_features=[] hardcoded, escondendo o erro real — foi exatamente
    // esse padrão que fez uma loja com o segmento certo no banco continuar
    // aparecendo como "generico" no painel, sem nenhum erro visível no
    // console (achado investigando um relato do Carlos). Agora essas colunas
    // críticas vêm isoladas: mesmo se as colunas "extras" abaixo falharem,
    // a navegação continua correta.
    const base = await pool.query(
      `SELECT id, name, slug, vertical_slug, active_features, business_config,
              setup_done, cor_primaria, cor_destaque, store_token
       FROM establishments WHERE id = $1`,
      [estId]
    );
    if (!base.rows.length) return res.status(404).json({ error: 'Estabelecimento não encontrado.' });

    const establishment: Record<string, any> = base.rows[0];

    // Colunas de perfil/contato — não devem derrubar a navegação se faltarem.
    try {
      const r = await pool.query(
        `SELECT description, category, city, phone, is_public, logo_url, cover_url,
                instagram_url, facebook_url, website_url, whatsapp_link
         FROM establishments WHERE id = $1`,
        [estId]
      );
      Object.assign(establishment, r.rows[0] || {});
    } catch (e: any) {
      console.error(`[lojista/me] colunas de perfil indisponíveis (est=${estId}):`, e.message);
    }

    // Colunas financeiras (Triple-Play — migration_v5, não roda sozinha, só
    // manual) — idem, não pode quebrar a navegação se o banco ainda não tiver.
    try {
      const r = await pool.query(
        `SELECT asaas_api_key, asaas_wallet_id, pix_key_type, pix_key_value,
                pix_receiver_name, preferred_payment_method, mp_status
         FROM establishments WHERE id = $1`,
        [estId]
      );
      Object.assign(establishment, r.rows[0] || {});
    } catch (e: any) {
      console.error(`[lojista/me] colunas financeiras indisponíveis (est=${estId}):`, e.message);
    }
    ...
    // caso é melhor devolver o erro real do que fingir sucesso com dados
    // errados (generico/[]), que é o que escondia esse tipo de bug antes.
    console.error(`[lojista/me] falha crítica (est=${estId}):`, err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/assistente/dica', async (req, res) => {
  ...
  try {
    ...
    const result = await askAI(cfg, [
      {
        role: 'system',
        content: `Você é a Ágata, assistente inteligente da plataforma Águia-ON para gestão de negócios.
