...
(async () => {
  try {
    ...
    // que já existisse no banco. Em um banco novo isso quebra o boot inteiro
    // do módulo de delivery. Cria aqui, idempotente, antes dos ALTERs.
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_user_addresses_user ON user_addresses(user_id)`);
    ...
    // business_hours é outro caso do mesmo problema: usada (SELECT/INSERT) aqui
    // em delivery.ts, em shared/businessHours.ts, em shared/syncAgata.ts e em
    // GET /lojista/onboarding-status, mas nunca teve CREATE TABLE em lugar
    // nenhum do projeto — achado ao investigar um 500 real em
    // /lojista/onboarding-status ("relation business_hours does not exist")
    // reportado pelo Carlos. Cria aqui, idempotente, mesmo padrão de user_addresses.
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_business_hours_estab ON business_hours(establishment_id)`);
...
<p class="footer">Águia-ON Food &bull; Obrigado pela preferência!</p>
...
// GET /delivery/public/my-orders — todos os pedidos do telefone em qualquer loja (para o portal Águia-ON)
