...
import { getDeviceLocation } from '../shared/gpswoxClient';
...
// ─────────────────────────────────────────────────────────────
// GET /client/rastreamento — veículos e faturas do cliente logado no módulo
// de Rastreamento (Fase 14). Um cliente pode estar vinculado a mais de uma
// empresa (ex: rastreamento em duas frotas diferentes), por isso agrupa por
// loja em vez de assumir uma só.
// ─────────────────────────────────────────────────────────────
router.get('/rastreamento', async (req, res) => {
  const user = req.user as TokenPayload;
  if (user.role !== 'CLIENT') return res.status(403).json({ error: 'Apenas clientes.' });
  try {
    const clientesRes = await pool.query(
      `SELECT c.id, c.nome, c.establishment_id, e.name AS establishment_name, e.logo_url AS establishment_logo,
              c.recorrencia_ativa, c.valor_recorrente, c.descricao_recorrente
    ...
    const lojas = await Promise.all(clientesRes.rows.map(async (c) => {
      const [veiculos, faturas] = await Promise.all([
        pool.query(
          `SELECT id, placa, modelo, ano, cor, status,
                  (gpswox_device_id IS NOT NULL) AS tem_rastreador
           FROM agenda_frota WHERE cliente_id=$1 ORDER BY placa`,
          [c.id]
        ),
        pool.query(
          `SELECT id, valor, status, vencimento, descricao, invoice_url, pix_payload, synced_at
           FROM agenda_cliente_asaas_cache WHERE cliente_id=$1 AND tipo='payment' ORDER BY synced_at DESC LIMIT 20`,
          [c.id]
        ),
      ]);
      return {
        ...
        establishment_logo: c.establishment_logo,
        cliente_id: c.id,
        recorrencia: c.recorrencia_ativa ? { valor: c.valor_recorrente, descricao: c.descricao_recorrente } : null,
...
// GET /client/rastreamento/veiculos/:veiculoId/localizacao — posição atual
// via GPSWOX, sob demanda (mesmo padrão de "abrir status" já usado no painel
// do lojista em GET /agenda/frota/:id/localizacao — sem mapa embutido, sem
// polling automático, só busca quando o cliente clica). Ownership checada
// via agenda_clientes.user_id (o vínculo de login criado em
// findOrCreateClienteUser, Fase 14) — não basta ter o id do veículo, o
// veículo precisa pertencer a um cliente vinculado a esse usuário logado.
router.get('/rastreamento/veiculos/:veiculoId/localizacao', async (req, res) => {
  const user = req.user as TokenPayload;
  if (user.role !== 'CLIENT') return res.status(403).json({ error: 'Apenas clientes.' });
  try {
    const veh = await pool.query(
      `SELECT f.gpswox_device_id, f.establishment_id
       FROM agenda_frota f
       JOIN agenda_clientes c ON c.id = f.cliente_id
       WHERE f.id = $1 AND c.user_id = $2`,
      [req.params.veiculoId, user.userId]
    );
    if (!veh.rows.length) return res.status(404).json({ error: 'Veículo não encontrado.' });
    if (!veh.rows[0].gpswox_device_id) {
      return res.status(422).json({ error: 'Esse veículo ainda não tem rastreador vinculado.' });
    }
