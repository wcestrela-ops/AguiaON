...
async function findEntity(identifier: string): Promise<EntityResult | null> {
  ...
  // 1. SuperAdmin (identificado pelo WA do .env)
  const adminWa = (process.env.ADMIN_WHATSAPP || '').replace(/\D/g, '');
  if (adminWa && wa && wa === adminWa) {
...
router.post('/login/verify', authLimiter, async (req, res) => {
  ...
  const { wa } = parseIdentifier(identifier);
  const secret = process.env.JWT_SECRET;
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: 'superadmin', whatsapp: wa, role: 'SUPERADMIN' as UserRole, establishmentId: null },
...
router.post('/forgot-password', authLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!entity || entity.type === 'SUPERADMIN') {
      return res.json({ sent: true, channels: [] });
    }

    const result = await sendOtp(entity.id, entity.whatsapp || '', entity.email, 'PASSWORD_RESET');
...
router.post('/reset-password', authLimiter, async (req, res) => {
  ...
  try {
    ...
    const valid = await verifyOtp(entity.id, code, 'PASSWORD_RESET');
    if (!valid) return res.status(401).json({ error: 'Código inválido ou expirado.' });
    ...
    const token = issueToken(entity);
    setAuthCookie(res, token, entity.type);
    return res.json({ success: true, token, role: entity.type, redirect: '/portal' });
